 ...
 */
 ...
 // Binance Testnet Provider
 const binanceTestnetProvider = new HDWalletProvider({
   privateKeys: [process.env.SECRET_WALLET],
...
// })
 ...
 const ropstenProvider = new HDWalletProvider({
   privateKeys: [process.env.SECRET_WALLET],
   providerOrUrl: process.env.PROVIDER_ROPSTEN 
 })

 const kovanProvider = new HDWalletProvider({
  privateKeys: [process.env.SECRET_WALLET],
  providerOrUrl: process.env.PROVIDER_KOVAN 
 })
 ...
 module.exports = {
   ...
   networks: {
     ...
     //
     development: {
      provider: () => new HDWalletProvider(process.env.GANACHE_SEED_PHASE, process.env.PROVIDER_LOCAL), 
     ...
     kovan: {
        ...
        timeoutBlocks: 60000,  // # of blocks before a deployment times out  (minimum/default: 50)
     ...
     ropsten: {
        ...
        gas: 7992189,        // Ropsten has a lower block limit than mainnet
   ...
   compilers: {
     solc: {
       version: process.env.COMPILER_VERSION,    // Fetch exact version from solc-bin (default: truffle's version)
